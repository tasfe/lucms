 ...
 */
session_start();
$access = isset($_SESSION['apppath']);
if(empty($access)) exit("{'err':'".jsonString("无权限")."','msg':'无权限'}");
$serip = $_SESSION['wwwroot'].'/Uploads/';
	...
	if(preg_match('/'.str_replace(',','|',$upext).'/i',$extension))
	{
		...
		else
		{
			...
			if($msgtype==1)$msg="'$target'";
				...
				$target = $serip.$target;
				$msg="{'url':'".$target."','localname':'".jsonString($upfile['name'])."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
			}
		}
	}
